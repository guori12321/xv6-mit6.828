...
env_setup_vm(struct Env *e)
{
    ...
    // inc pp_ref for env_pgdir
    page_incref(p);
    // set e
    e->env_cr3 = page2pa(p);
    e->env_pgdir = KADDR(e->env_cr3);
    // init pgdir
    memmove((void *)(e->env_pgdir), (void *)boot_pgdir, PGSIZE);
    memset((void *)(e->env_pgdir), 0, PDX(UTOP) * sizeof(e->env_pgdir[0]));
...
segment_alloc(struct Env *e, void *va, size_t len)
{
    ...
    //Allocate pages for env
    void * vend = ROUNDUP(va + len, PGSIZE);
    va = ROUNDDOWN(va, PGSIZE);
    struct Page *p = NULL;
    for (; va < vend; va += PGSIZE) {
        if (page_alloc(&p) < 0)
            panic("segment_alloc fail!");
        if (0 != page_insert(e->env_pgdir, p, va, PTE_W | PTE_U))
            panic("segment_alloc fail!\n");
        }
}
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
    ...
    for (; ph < eph; ++ph)
        if (ELF_PROG_LOAD == ph->p_type) {
            ...
            void *vm;
            for (vm =(void *)ROUNDDOWN(ph->p_va, PGSIZE); vm < (void *)ROUNDUP((ph->p_va + ph->p_memsz), PGSIZE); vm += PGSIZE) {
                pte_t *ptevm = pgdir_walk(e->env_pgdir, vm, 0);
                void *dstvm = (void *)KADDR(PTE_ADDR(*ptevm));
                memset(dstvm, 0, PGSIZE);
            }

            if (0 < ph->p_memsz) {
                void * src = ph->p_offset + binary, *dst;
                void * va = (void *)ph->p_va;
                int sf, sc, sm;
                for (sf = ph->p_filesz; sf > 0; sf -= sc, src += sc, va += sc) {
                    sc = (PGSIZE - PGOFF(va)) < sf ? (PGSIZE - PGOFF(va)) : sf;
                    pte_t * pte = pgdir_walk(e->env_pgdir, va, 0);
                    dst = (void *)KADDR(PTE_ADDR(*pte)) + PGOFF(va);
                    memmove(dst, src, sc);
                }
            }
        }
