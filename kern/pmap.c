...
i386_vm_init(void)
{
    ...
    pages = (struct Page *)boot_alloc(sizeof(pages[0]) * npage, PGSIZE);

	//////////////////////////////////////////////////////////////////////
	// Make 'envs' point to an array of size 'NENV' of 'struct Env'.
	// LAB 3: Your code here.

    envs = (struct Env *)boot_alloc(sizeof(envs[0]) * NENV, PGSIZE);
    ...
    boot_map_segment(pgdir, UPAGES, ROUNDUP(sizeof(pages[0]) * npage, PGSIZE), 
                        PADDR(pages), PTE_U);
	//////////////////////////////////////////////////////////////////////
    ...
    boot_map_segment(pgdir, UENVS, ROUNDUP(sizeof(envs[0]) * NENV, PGSIZE), 
                        PADDR(envs), PTE_U);
    ...
    boot_map_segment(pgdir, KSTACKTOP - KSTKSIZE, ROUNDUP(KSTKSIZE, PGSIZE),
    ...
    boot_map_segment(pgdir, KERNBASE, ROUNDDOWN(0xFFFFFFFF - KERNBASE, PGSIZE), 0, PTE_W);
...
is_sys_use(int pn)
{
    physaddr_t addr = pn * PGSIZE;
    if (0 == addr)
        return true;
    else if ((IOPHYSMEM <= addr) && (addr < EXTPHYSMEM))
        return true;
    else if ((PADDR(0xF0100000) <= addr) && (addr < ROUNDUP(PADDR(boot_freemem), PGSIZE)))
        return true;
...
{
    struct Page * fp = LIST_FIRST(&page_free_list);
    if (fp) {
        assert(page2pa(fp) != PADDR(boot_pgdir));
        LIST_REMOVE(fp, pp_link);
        page_initpp(fp);
        *pp_store = fp;
        return 0;
    }
...
void
page_clear(struct Page * pag)
{
    memset((void *)KADDR(page2pa(pag)), 0, PGSIZE);
}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    if (0 == *pgdir) {
        ...
        pag->pp_ref = 1;
        page_clear(pag);
        *pgdir = page2pa(pag) | PTE_USER | PTE_P;
    }

    return KADDR((physaddr_t)((pte_t *)PTE_ADDR(*pgdir) + PTX(va)));
}
...
boot_map_segment(pde_t *pgdir, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
    ...
    uintptr_t va;
    for (va = la; va < la + size; va += PGSIZE, pa += PGSIZE) {
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    for (va += PGSIZE; va < vaend; va += PGSIZE) {
            pte = pgdir_walk(env->env_pgdir, va, 0);
            if (!pte || ((*pte) & perm) != perm) {
                user_mem_check_addr = (uintptr_t)PTE_ADDR(va);
                return -E_FAULT;
            }
    }

    pte = pgdir_walk(env->env_pgdir, vaend - 1, 0);
    if (!pte || ((*pte) & perm) != perm) {
        user_mem_check_addr = (uintptr_t)PTE_ADDR(vaend);
        return -E_FAULT;
    }
